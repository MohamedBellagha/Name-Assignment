    ...
    if os.path.exists(dat_fname):
        ...
        for fname in fnames:
            ...
            for i in range(0, len(lines), 3):
                ...
                # text += text_raw + " "
                text += (lines[i].replace("$ه$", lines[i + 1].strip())).strip()
    ...
    def __init__(self, fname, tokenizer):
        ...
        for i in range(0, len(lines), 3):
            text_left, _, text_right = [s.lower().strip()
                                        for s in lines[i].partition("$ه$")]
            aspect = lines[i + 1].lower().strip()
            text_left = (text_left.replace(
                "$ه$", lines[i + 1].strip())).strip()
            text_right = (text_right.replace(
                "$ه$", lines[i + 1].strip())).strip()
            ...
            context_indices = tokenizer.text_to_sequence(
                lines[i].replace("$ه$", ''))  # text_left + " " + text_right)
