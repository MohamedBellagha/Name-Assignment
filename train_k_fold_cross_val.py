...
class Instructor:
    def __init__(self, opt):
        ...
        #else:
        tokenizer = build_tokenizer(fnames=[opt.dataset_file['train'], opt.dataset_file['test']],max_seq_len=opt.max_seq_len,dat_fname='{0}_tokenizer.dat'.format(opt.dataset))
        embedding_matrix = build_embedding_matrix(word2idx=tokenizer.word2idx,embed_dim=opt.embed_dim,dat_fname='{0}_{1}_embedding_matrix.dat'.format(str(opt.embed_dim), opt.dataset))
        self.model = opt.model_class(embedding_matrix, opt).to(opt.device)
##

        self.trainset = ABSADataset(opt.dataset_file['train'], tokenizer)
        #self.testset = ABSADataset(opt.dataset_file['test'], tokenizer)
        valset_len = int(len(self.trainset) * 0.15)
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            val_acc, val_f1, val_precision, val_recall,val_f1_macro,val_precision_macro,val_recall_macro= self._evaluate_acc_f1(val_data_loader)
            logger.info('> val_acc: {:.4f}, val_f1: {:.4f}, val_precision: {:.4f},val_recall: {:.4f},val_f1_macro: {:.4f},val_precision_macro: {:.4f},val_recall_macro: {:.4f}'.format(val_acc, val_f1, val_precision, val_recall,val_f1_macro,val_precision_macro,val_recall_macro))
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        f1 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1, 2, 3], average='weighted')
        precision = metrics.precision_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), average='weighted')
        recall = metrics.recall_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), average='weighted')
        ##
    ...
    def run(self):
        ...
        all_test_acc, all_test_f1,all_test_precision,all_test_recall,all_test_f1_macro, all_test_precision_macro, all_test_recall_macro= [], [],[],[],[],[],[]
        for fid in range(self.opt.cross_val_fold):
            ...
            test_acc, test_f1, test_precision, test_recall,test_f1_macro,test_precision_macro,test_recall_macro= self._evaluate_acc_f1(test_data_loader)
            all_test_acc.append(test_acc)
            all_test_f1.append(test_f1)
            all_test_precision.append(test_precision)
            all_test_recall.append(test_recall)
            all_test_f1_macro.append(test_f1_macro)
            all_test_precision_macro.append(test_precision_macro)
            all_test_recall_macro.append(test_recall_macro)
            logger.info('>> test_acc: {:.4f}, test_f1: {:.4f}, test_precision: {:.4f}, test_recall: {:.4f}, test_f1_macro: {:.4f}, test_precision_macro: {:.4f}, test_recall_macro: {:.4f}'.format(test_acc, test_f1, test_precision, test_recall,test_f1_macro,test_precision_macro,test_recall_macro))

        mean_test_acc, mean_test_f1,mean_test_precision,mean_test_recall,mean_test_f1_macro, mean_test_precision_macro, mean_test_recall_macro = numpy.mean(all_test_acc), numpy.mean(all_test_f1), numpy.mean(all_test_precision),numpy.mean(all_test_recall),numpy.mean(all_test_f1_macro),numpy.mean(all_test_precision_macro),numpy.mean(all_test_recall_macro)
        logger.info('>' * 100)
        logger.info('>>> mean_test_acc: {:.4f}, mean_test_f1: {:.4f},mean_precision:{:.4f},mean_recall{:.4f},mean_f1_macro:{:.4f},mean_precision_macro:{:.4f},mean_recall_macro: {:.4f}'.format(mean_test_acc, mean_test_f1,mean_test_precision,mean_test_recall,mean_test_f1_macro, mean_test_precision_macro, mean_test_recall_macro))
...
def main():
    ...
    input_colses = {
        ...
        'aoaCNN': ['text_indices', 'aspect_indices'],
