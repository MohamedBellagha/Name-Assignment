...
class Instructor:
    ...
    def _evaluate_acc_f1(self, data_loader,test_mod=False):
        ...
        if(test_mod==True):
            ...
            logger.info('{}'.format((torch.argmax(t_outputs_all, -1).cpu()).tolist()))
            logger.info('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
            logger.info('{}'.format((t_targets_all.cpu()).tolist()))
    ...
    def run(self):
        ...
        test_acc, test_f1,precision,recall,test_f1_macro,test_precision_macro,test_recall_macro = self._evaluate_acc_f1(test_data_loader,test_mod=True)
...
def main():
    ...
    parser.add_argument('--dataset', default='speech_turns',type=str)
