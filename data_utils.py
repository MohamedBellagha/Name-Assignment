...
def build_embedding_matrix(word2idx, embed_dim, dat_fname):
    if os.path.exists(dat_fname):
        ...
        fname = './glove.twitter.27B/glove.twitter.27B.' + str(embed_dim) + 'd.txt' \
            if embed_dim != 300 else './fastext.txt'
        word_vec = _load_word_vec(
            fname, word2idx=word2idx, embed_dim=embed_dim)
        print(fname)
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        for i in range(0, len(lines), 3):
            ...
            context_indices = tokenizer.text_to_sequence(
                text_left + " " + text_right)
            left_indices = tokenizer.text_to_sequence(text_left)
            left_with_aspect_indices = tokenizer.text_to_sequence(
                text_left + " " + aspect)
            right_indices = tokenizer.text_to_sequence(
                text_right, reverse=True)
            right_with_aspect_indices = tokenizer.text_to_sequence(
                aspect + " " + text_right, reverse=True)
            aspect_indices = tokenizer.text_to_sequence(aspect)
            #############
            ddr = right_with_aspect_indices
            nw = left_indices
            ll = [0, 0, 0, 0, 0, 0, 0]
            for i in nw:
                if i > 0:
                    ll.append(i)
            ll = ll[len(ll)-7:len(ll)]
            np_ind_left = np.array(ll)
            np_ind_right_aspect = ddr[0:7]
            cnn_aspect_indices = np.concatenate(
                [np_ind_left, np_ind_right_aspect])
            ###########
            left_len = np.sum(left_indices != 0)
            aspect_len = np.sum(aspect_indices != 0)
            aspect_CNN_len = np.sum(cnn_aspect_indices != 0)
            aspect_boundary = np.asarray(
                [left_len, left_len + aspect_len - 1], dtype=np.int64)
            polarity = int(polarity) + 1

            text_len = np.sum(text_indices != 0)
            concat_bert_indices = tokenizer.text_to_sequence(
                '[CLS] ' + text_left + " " + aspect + " " + text_right + ' [SEP] ' + aspect + " [SEP]")
            concat_segments_indices = [0] * \
                (text_len + 2) + [1] * (aspect_len + 1)
            concat_segments_indices = pad_and_truncate(
                concat_segments_indices, tokenizer.max_seq_len)
            ...
            data = {
                'cnn_aspect_indices': cnn_aspect_indices,
